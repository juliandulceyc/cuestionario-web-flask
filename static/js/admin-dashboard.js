...
const Utils = {
  ...
  escapeAttr: function (text) {
    ...
    return String(text).replaceAll(/[&"'<>]/g, function (m) { return map[m]; });
  },
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setupTemaHandling() {
    ...
    if (formSubirTema) {
      formSubirTema.addEventListener('submit', handleSubirTema);
    }
  }
  ...
  function removeCandidatoUI(codigo, cardElement) {
        ...
        .then(data => {
          if (data.success) {
            ...
            if (cardElement) {
              ...
              setTimeout(removeCandidatoUI, 300, codigo, cardElement);
            }
          } else {
...
globalThis.abrirEditarCandidato = function (c) {
  ...
  if (!modal) {
    Utils.mostrarNotificacion('Modal #modal-editar no encontrado en el DOM', ERROR);
    return;
  }
...
globalThis.abrirEditarCandidatoDesdeBoton = function (btn) {
  ...
  for (const key of ['codigo', 'tipo_documento', 'numero_documento', 'nombre_completo', 'email', 'telefono', 'cargo']) {
    ...
    let val = btn.getAttribute(`data-${key.replaceAll('_', '-')}`);
    if (val === null) val = '';
    c[key] = val;
  }
